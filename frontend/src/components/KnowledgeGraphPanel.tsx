...
function wrapText(str: string, maxLen: number = 10): string {
  if (!str) return '';
  if (str.length <= maxLen) return str;
  return str.substring(0, maxLen) + '\n' + wrapText(str.substring(maxLen), maxLen);
}
...
const KnowledgeGraphPanel: React.FC<KnowledgeGraphPanelProps> = ({
  ...
  const loadGraphData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await knowledgeGraphApi.getGraphData(bookTitle);
      ...
      const nodeIds = new Set(g6Nodes.map(n => n.id));
      ...
      const g6Edges = edges
        ...
        .map((e: GraphEdgeData, i: number) => ({
          ...
          data: {
            ...
            isDashed: e.edge_type !== 'CHAPTER_SEQUENCE',
          },
        }));
  ...
  }, [initGraph, loadGraphData, checkHealth]);
  ...
  const handleDeleteNode = async () => {
    ...
    if (!confirm(`确定要删除节点 "${selectedNode.name}" 吗？`)) return;
  ...
  return (
    <div className="knowledge-graph-panel">
      ...
      <div className="kg-legend">
        {Object.entries(ENTITY_COLORS_HEX).slice(0, 6).map(([type, color]) => (
          <div key={type} className="kg-legend-item">
            <span className="kg-legend-dot" style={{ backgroundColor: color }} />
            <span>{type}</span>
          </div>
        ))}
      </div>
