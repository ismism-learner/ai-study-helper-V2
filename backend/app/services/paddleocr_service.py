...
class PaddleOCRService:
    ...
    def _do_load_model(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"PaddleOCR model loaded successfully on GPU in {total_time:.2f}s"
            )
            return True
    ...
    async def process_image(self, image_path: str) -> PaddleOCRResult:
        ...
        """
        if not os.path.exists(image_path):
            return PaddleOCRResult(success=False, error=f"图片文件不存在: {image_path}")

        if not self.model_loaded:
            success = await self.load_model()
            if not success:
                return PaddleOCRResult(
                    success=False, error=f"模型加载失败: {self._load_error}"
                )

        try:
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                self._thread_pool, self._process_single_image_sync, image_path
            )
    ...
    async def process_pdf(
        ...
        if not os.path.exists(pdf_path):
            return PaddleOCRResult(success=False, error=f"PDF 文件不存在: {pdf_path}")
        ...
        try:
            ...
            for page_idx in range(start_page, actual_end):
                ...
                tmp_path = os.path.join(
                    tmp_dir, f"paddle_ocr_{uuid.uuid4().hex}_{page_idx}.png"
                )
                ...
                try:
                    ...
                    if result.success:
                        ...
                        logger.warning(
                            f"Failed to process page {page_idx + 1}: {result.error}"
                        )
                        pages_result.append(
                            {"page_number": page_idx + 1, "error": result.error}
                        )
        ...
        try:
            ...
            for page_idx in range(start_page, actual_end):
                ...
                try:
                    ...
                    logger.info(
                        f"OCR result for page {page_idx + 1}: success={result.success}"
                    )
                        ...
                        for ocr_item in result.ocr_results:
                            ...
                            try:
                                ...
                                text_instances.append(
                                    {"text": text, "rect": rect, "fontsize": fontsize}
                                )
                            except Exception as e:
            ...
            if has_text:
                ...
                for page_idx, page in enumerate(doc):
                    ...
                    if text.strip():
                        ...
                        pages_text.append(
                            {"page_number": page_idx + 1, "text": text.strip()}
                        )
                ...
                logger.info(
                    f"Extracted text from PDF text layer, saved to: {text_file_path}"
                )
        ...
        try:
            ...
            if status_callback:
                await status_callback(
                    "processing", 0, f"正在处理 {total_pages_to_process} 页..."
                )
                ...
                with completed_lock:
                    ...
                    if progress_callback:
                        ...
                        await progress_callback(
                            progress, completed_count, total_pages_to_process
                        )
            ...
            all_text = [
                r["text"] for r in results_list if r.get("success") and r.get("text")
            ]
            pages_text = [
                {"page_number": r["page_number"], "text": r["text"]}
