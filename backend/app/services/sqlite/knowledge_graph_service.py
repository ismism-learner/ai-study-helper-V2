...
from app.models import BookDocument, KnowledgeEdge, KnowledgeNode
...
class KnowledgeGraphService:
    ...
    def _update_nodes_parent_summary(
        self, book_id: str, new_summary_position: int, new_summary_id: str, new_chapter_index: int = None
    ):
        prev_summary = (
            ...
            if next_summary:
                ...
                for node in nodes_after_new:
                    if node.parent_summary_id == prev_summary.id:
                        ...
                        logger.info(
                            f"[QuickSummary] Reassigned DetailedQuestion: {node.name} from {prev_summary.name} to new summary"
                        )

            logger.info(f"[QuickSummary] prev_summary: {prev_summary.name}, chapter_index={prev_summary.chapter_index}")
            logger.info(
                f"[QuickSummary] new_summary: position={new_summary_position}, chapter_index={new_chapter_index}"
            )
    ...
    def search_nodes(self, keyword: str, entity_type: str = None, limit: int = 20) -> list[KnowledgeNode]:
        query = self.db.query(KnowledgeNode).filter(KnowledgeNode.name.contains(keyword))
        if entity_type:
    ...
    def get_graph_data(self, book_title: str = None) -> dict[str, Any]:
        if book_title:
            ...
            edges = (
                self.db.query(KnowledgeEdge)
                .filter(KnowledgeEdge.source_id.in_(node_ids), KnowledgeEdge.target_id.in_(node_ids))
                .all()
            )
        else:
    ...
    def get_graph_data_by_tag(self, tag: str) -> dict[str, Any]:
        ...
        books = self.db.query(BookDocument).filter(BookDocument.tags.contains(f'"{tag}"')).all()
        ...
        nodes = self.db.query(KnowledgeNode).filter(KnowledgeNode.book_id.in_(book_ids)).all()
        ...
        return {
            ...
            "books": [{"id": b.id, "title": b.title, "cover": b.cover_image} for b in books],
    ...
    def update_node(self, node_id: str, name: str = None, description: str = None) -> KnowledgeNode | None:
        node = self.get_node(node_id)
        if not node:
            return None
    ...
    def find_parent_summary(self, book_id: str, text_position: int) -> str | None:
        prev_summary = (
            self.db.query(KnowledgeNode)
            .filter(
                KnowledgeNode.book_id == book_id,
    ...
    def clear_by_book(self, book_id: str) -> dict:
        """清除指定书籍的所有知识图谱数据"""
        # Delete edges first (foreign key constraint)
        nodes = self.db.query(KnowledgeNode).filter(KnowledgeNode.book_id == book_id).all()
        node_ids = [n.id for n in nodes]
        edges_deleted = (
            self.db.query(KnowledgeEdge)
            .filter(
        ...
        nodes_deleted = (
            self.db.query(KnowledgeNode).filter(KnowledgeNode.book_id == book_id).delete(synchronize_session=False)
        )
        self.db.commit()
