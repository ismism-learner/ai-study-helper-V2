    ...
    for attempt in range(max_retries):
        try:
            ...
            is_retryable = any(code in error_str for code in [
                '503', 'overloaded', 'memory', 'rate limit', '500', 
                'internal error', 'timeout', 'connection', 'timed out',
                '429', 'too many requests', 'service unavailable',
                'bad gateway', 'gateway timeout', '502', '504',
                'network', 'socket', 'eof', 'reset', 'broken pipe'
            ])
            ...
            if 'api key' in error_str or 'authentication' in error_str or 'unauthorized' in error_str:
                ...
                raise ValueError(f"模型不存在：请检查模型名称 '{settings_manager.model_name}' 是否正确。错误详情：{str(e)}")
            elif 'context' in error_str and 'length' in error_str:
                raise ValueError(f"内容过长：输入内容超出模型上下文限制。请尝试缩短内容或使用支持更长上下文的模型。")
...
class AIService:
    ...
    def update_client(self):
        ...
        print(f"[AIService] Updating client:")
        print(f"  - Base URL (normalized): {base_url}")
        print(f"  - API Key (first 10 chars): {api_key[:10] if api_key else 'NOT SET'}...")
        print(f"  - Model: {settings_manager.model_name}")
...
- 术语罗列要全面"""
    ...
    async def generate_text(self, prompt: str, system_prompt: str = None) -> str:
        ...
        if len(prompt) > MAX_PROMPT_LENGTH:
            print(f"[generate_text] WARNING: Prompt length ({len(prompt)}) exceeds recommended limit ({MAX_PROMPT_LENGTH})")
        ...
        async def _call_api():
            response = await self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": system_prompt or "你是一个专业的写作助手，擅长生成结构清晰、逻辑连贯的文章。"
                    },
                    {"role": "user", "content": prompt}
                ],
                temperature=0.7
    ...
    async def generate_text_stream(
        ...
        MAX_PROMPT_LENGTH = 30000
        if len(prompt) > MAX_PROMPT_LENGTH:
            print(f"[generate_text_stream] WARNING: Prompt length ({len(prompt)}) exceeds recommended limit ({MAX_PROMPT_LENGTH})")
    ...
    async def generate_timeline_notes(
        ...
        default_prompt = """请仔细阅读以下文档内容，提取其中提到的所有历史事件和时间节点。

要求：
1. 识别文档中所有明确提到的时间点（年份、年月、或具体日期）
2. 为每个时间点提取对应的事件标题和简短描述
3. 按时间顺序排列事件

输出格式规范：
- 单个事件格式：[YYYY-MM-DD/事件标题/简短内容解释]
- 多个事件格式：{[时间1/内容1], [时间2/内容2], [时间3/内容3]}
- 时间格式说明：
  - 只有年份：[1995/事件标题/描述]
  - 有年月：[1995-03/事件标题/描述]
  - 有年月日：[1995-03-15/事件标题/描述]
- 公元前年份用负数表示，如：[-221/秦统一六国/秦始皇完成统一大业]

注意事项：
1. 只提取文档中明确提到的时间，不要推测
2. 如果只提到年份没有月份日期，就只写年份
3. 事件描述要简洁，不超过50字
4. 保持客观，使用原文描述
