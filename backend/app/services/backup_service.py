...
DB_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "interactive_docs.db")
DATA_BACKUP_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "data_backup.json")
...
class BackupService:
    ...
    def _get_db_stats(self, db_path: str) -> Dict[str, int]:
        ...
        try:
            ...
            tables = [
                ...
                "countries", "categories", "time_periods", "book_time_periods"
            ]
    ...
    def _save_backup_manifest(self, backup_name: str, reason: str, stats: Dict[str, int]):
    ...
    def restore_backup(self, backup_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Restored backup: {backup_name}, stats: {result['restored_stats']}")
    ...
    def check_data_integrity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if docs_count < MIN_DOCUMENTS_WARNING:
                result["warnings"].append(f"文档数量较少 ({docs_count})，可能存在数据丢失")
                result["recommendations"].append("建议检查最近的备份并考虑恢复")
                result["healthy"] = False
            
            if books_count < MIN_DOCUMENTS_WARNING:
                result["warnings"].append(f"书籍数量较少 ({books_count})，可能存在数据丢失")
                result["recommendations"].append("建议检查最近的备份并考虑恢复")
                result["healthy"] = False
