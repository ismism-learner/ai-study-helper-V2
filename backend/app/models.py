...
class KnowledgeNode(Base):
    ...
    book = relationship("BookDocument", foreign_keys=[book_id])
    parent_summary = relationship(
        "KnowledgeNode", remote_side=[id], foreign_keys=[parent_summary_id]
    )
...
class CognitiveChain(Base):
    ...
    book = relationship("BookDocument", foreign_keys=[book_id])
    nodes = relationship(
        "CognitiveNode", back_populates="chain", cascade="all, delete-orphan"
    )
...
class CognitiveNode(Base):
    ...
    id = Column(String, primary_key=True, default=generate_uuid)
    chain_id = Column(
        String, ForeignKey("cognitive_chains.id"), nullable=False, index=True
    )
    ...
    parent_node_id = Column(
        String, ForeignKey("cognitive_nodes.id"), nullable=True, index=True
    )
    ...
    parent = relationship(
        "CognitiveNode", remote_side=[id], foreign_keys=[parent_node_id]
    )
    book = relationship("BookDocument", foreign_keys=[book_id])
