        ...
        try:
            ...
            if existing_by_hash:
                return DuplicateCheckResult(
                    ...
                    details={"match_type": "file_hash", "file_hash": file_hash},
                )
        except Exception as e:
            print(f"Error calculating file hash: {e}")

        try:
            content_hash, text_length = duplicate_detector.calculate_content_hash(
                file_path
            )
...
def compute_hashes_for_all(
    ...
    books = db.query(BookDocument).filter(BookDocument.file_hash_sha256 == None).all()

    results = {"total": len(books), "processed": 0, "errors": []}

    for book in books:
        try:
            if not book.file_path or not os.path.exists(book.file_path):
                results["errors"].append(
                    {"book_id": book.id, "title": book.title, "error": "File not found"}
                )
                continue
            ...
            results["errors"].append(
                {"book_id": book.id, "title": book.title, "error": str(e)}
            )
        ...
        "groups": [
            {
                "group_id": group_id,
                "books": books,
                "primary_book_id": next(
                    (b["id"] for b in books if b["is_primary"] == 1),
...
def delete_duplicate_book(
    book_id: str, delete_file: bool = False, db: Session = Depends(get_db)
):
    book = db.query(BookDocument).filter(BookDocument.id == book_id).first()
    if not book:
