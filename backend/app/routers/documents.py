...
from app.schemas import (
    ...
    HighlightCreate, HighlightResponse, ExplainRequest, ExplainResponse,
    ParagraphOptimizeRequest, ParagraphOptimizeResponse,
    NotePolishRequest, NotePolishResponse,
    DocumentTimelineEventCreate, DocumentTimelineEventUpdate, DocumentTimelineEventResponse
)
...
async def upload_document_with_path(
    ...
    file_ext = os.path.splitext(file.filename)[1].lower()
    if file_ext not in ['.md', '.markdown', '.docx', '.doc', '.pdf', '.txt']:
        raise HTTPException(status_code=400, detail=f"Only MD, DOCX, DOC, PDF and TXT files are supported, got {file_ext}")
...
async def create_folder_structure_from_path(relative_path: str, db: Session) -> str:
    ...
    for folder_name in folder_parts:
        ...
        if existing_folder:
            ...
            print(f"创建文件夹: {folder_name} (id: {current_folder_id}, parent_id: {parent_id})")
...
async def upload_document(
    ...
    if file_ext not in ['.md', '.markdown', '.docx', '.pdf', '.txt']:
        raise HTTPException(status_code=400, detail=f"不支持的格式: {file_ext}，只支持 .md, .docx, .pdf, .txt")
...
def get_timeline_tags_history(db: Session = Depends(get_db)):
    ...
    doc_events = db.query(DocumentTimelineEvent.tags).filter(
        DocumentTimelineEvent.tags != None
    ).all()
    ...
    world_events = db.query(WorldTimelineEvent.tags).filter(
        WorldTimelineEvent.tags != None
    ).all()
    ...
    # 3. 从 Document 获取标签
    documents = db.query(Document.tags).filter(
        Document.tags != None
    ).all()
    for doc_tags in documents:
        if doc_tags[0]:
    ...
    books = db.query(BookDocument.tags).filter(
        BookDocument.tags != None
    ).all()
...
async def generate_framework(doc_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        print(f"[generate_framework] Successfully generated content for document: {doc.title}")
        return doc
        ...
        print(f"Error generating framework for document {doc_id}: {error_type}: {error_msg}")
        traceback.print_exc()
        
        if "timeout" in error_msg.lower() or "timed out" in error_msg.lower():
            raise HTTPException(status_code=504, detail=f"请求超时：模型响应时间过长，请尝试缩短内容或稍后重试。详情：{error_msg}")
        elif "api key" in error_msg.lower() or "authentication" in error_msg.lower():
            raise HTTPException(status_code=401, detail=f"API认证失败：请检查API Key设置。详情：{error_msg}")
        elif "model" in error_msg.lower() and "not found" in error_msg.lower():
            raise HTTPException(status_code=400, detail=f"模型不存在：请检查模型名称设置。详情：{error_msg}")
        elif "context" in error_msg.lower() and "length" in error_msg.lower():
            raise HTTPException(status_code=400, detail=f"内容过长：超出模型上下文限制。详情：{error_msg}")
        else:
...
def _save_framework_content(doc_id: str, content: str) -> bool:
    ...
    try:
        ...
        if doc:
            ...
            db_session.commit()
            print(f"[_save_framework_content] Successfully saved {len(content)} chars for doc: {doc_id}")
            return True
        else:
...
async def generate_framework_stream(doc_id: str, db: Session = Depends(get_db)):
    ...
    async def event_generator():
        ...
        try:
            ...
            print(f"[generate_framework_stream] Successfully generated content for: {doc_title}")
            data = json.dumps({"content": "", "done": True, "full_content": full_content}, ensure_ascii=False)
            yield f"data: {data}\n\n"
...
async def update_document(doc_id: str, doc_update: DocumentUpdate, db: Session = Depends(get_db)):
    doc = db.query(Document).filter(Document.id == doc_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
...
# 文档时间轴事件 API
@router.get("/documents/{doc_id}/timeline-events", response_model=List[DocumentTimelineEventResponse])
def get_document_timeline_events(
    ...
    query = db.query(DocumentTimelineEvent).filter(DocumentTimelineEvent.document_id == doc_id)

    if sort_by == "date":
        query = query.order_by(
            DocumentTimelineEvent.event_date.asc() if order == "asc" else DocumentTimelineEvent.event_date.desc()
        )
    elif sort_by == "created":
        query = query.order_by(
            DocumentTimelineEvent.created_at.asc() if order == "asc" else DocumentTimelineEvent.created_at.desc()
        )
...
@router.post("/documents/{doc_id}/timeline-events", response_model=DocumentTimelineEventResponse)
def create_document_timeline_event(
    doc_id: str, 
    data: DocumentTimelineEventCreate, 
    db: Session = Depends(get_db)
):
    """为文档创建时间轴事件"""
    doc = db.query(Document).filter(Document.id == doc_id).first()
...
@router.put("/document-timeline-events/{event_id}", response_model=DocumentTimelineEventResponse)
def update_document_timeline_event(
    event_id: str, 
    data: DocumentTimelineEventUpdate, 
    db: Session = Depends(get_db)
):
    """更新文档时间轴事件"""
    event = db.query(DocumentTimelineEvent).filter(DocumentTimelineEvent.id == event_id).first()
    if not event:
...
async def create_highlight(doc_id: str, highlight: HighlightCreate, db: Session = Depends(get_db)):
    doc = db.query(Document).filter(Document.id == doc_id).first()
    if not doc:
        raise HTTPException(status_code=404, detail="Document not found")
...
def update_highlight(highlight_id: str, update_data: dict, db: Session = Depends(get_db)):
    highlight = db.query(Highlight).filter(Highlight.id == highlight_id).first()
    if not highlight:
        raise HTTPException(status_code=404, detail="Highlight not found")
...
async def check_duplicates_by_name(
    request: dict,
    db: Session = Depends(get_db)
):
    """
...
async def batch_generate_content(
    ...
    print(f"[batch_generate_content] Starting batch generation for {len(document_ids)} documents")
    ...
    for i, doc_id in enumerate(document_ids):
        ...
        try:
            ...
            if doc.framework_content:
                print(f"[batch_generate_content] Document {doc.title} already has framework_content, skipping")
                results.append({
                    "id": doc_id,
                    "success": True,
                    "title": doc.title,
            ...
            content_to_process = doc.original_content[:15000]
            print(f"[batch_generate_content] Processing document {i+1}/{len(document_ids)}: {doc.title}")
            ...
            print(f"[batch_generate_content] Successfully generated content for: {doc.title}")
            ...
            error_msg = str(e)
            error_type = type(e).__name__
            print(f"Error generating content for document {doc_id}: {error_type}: {error_msg}")
            traceback.print_exc()
...
def parse_timeline_notes_output(output: str) -> List[TimelineNoteParseResult]:
    ...
    for match in matches:
        ...
        results.append(TimelineNoteParseResult(
...
@router.post("/documents/{doc_id}/ai-generate-timeline-notes", response_model=AIGenerateTimelineNotesResponse)
async def ai_generate_timeline_notes(
    ...
    content_to_process = request.content if request and request.content else doc.original_content
...
async def ai_generate_timeline_notes_stream(
    ...
    async def event_generator():
        ...
        try:
            ...
            print(f"[ai_generate_timeline_notes_stream] Parsed {len(parsed_events)} events")
            data = json.dumps({
                "content": "", 
                "done": True, 
...
async def save_timeline_notes_batch(
    doc_id: str,
    request: SaveTimelineNotesBatchRequest,
    db: Session = Depends(get_db)
):
    """
    批量保存AI生成的时间笔记到数据库
...
def get_incomplete_generations(
    ...
    documents = db.query(Document).filter(
        Document.original_content != None,
    ...
    for doc in documents:
        ...
        if issues:
            incomplete_docs.append({
                "id": doc.id,
                "title": doc.title,
                "original_content_length": len(doc.original_content) if doc.original_content else 0,
                "framework_content_length": len(doc.framework_content) if doc.framework_content else 0,
                "issues": issues,
...
async def batch_regenerate_content(
    request: dict,
    db: Session = Depends(get_db)
):
    """
    ...
    print(f"[batch_regenerate] Starting batch regeneration for {len(document_ids)} documents")
    ...
    for i, doc_id in enumerate(document_ids):
        ...
        try:
            ...
            if doc.framework_content and not force_regenerate:
                print(f"[batch_regenerate] Document {doc.title} already has framework_content, skipping")
                results.append({
                    "id": doc_id,
                    "success": True,
                    "title": doc.title,
            ...
            content_to_process = doc.original_content[:15000]
            print(f"[batch_regenerate] Processing document {i+1}/{len(document_ids)}: {doc.title}")
            print(f"[batch_regenerate] Content length: {len(content_to_process)}, Old framework length: {old_length}")
            ...
            print(f"[batch_regenerate] Successfully regenerated content for: {doc.title} ({old_length} -> {len(generated_content)} chars)")
            ...
            error_msg = str(e)
            error_type = type(e).__name__
            print(f"Error regenerating content for document {doc_id}: {error_type}: {error_msg}")
            traceback.print_exc()
