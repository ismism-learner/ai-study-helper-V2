...
def get_quick_note_groups(db: Session = Depends(get_db)):
    groups = (
        db.query(QuickNote.group_id, QuickNote.group_name)
        .filter(QuickNote.group_id.isnot(None))
        .distinct()
        .all()
    )

    group_counts = dict(
        db.query(QuickNote.group_id, func.count(QuickNote.id))
        .filter(QuickNote.group_id.isnot(None))
        .group_by(QuickNote.group_id)
        .all()
    )
...
async def update_quick_note(
    note_id: str, note_update: QuickNoteUpdate, db: Session = Depends(get_db)
):
    note = db.query(QuickNote).filter(QuickNote.id == note_id).first()
    if not note:
        raise HTTPException(status_code=404, detail="Quick note not found")
...
async def batch_process_quick_notes(
    request: QuickNoteBatchProcessRequest, db: Session = Depends(get_db)
):
    results = []
    success_count = 0
    failed_count = 0
...
只输出标签，不要其他内容："""
                    ...
                    suggested_tags = [
                        t.strip() for t in tags_response.split(",") if t.strip()
                    ]
...
async def convert_quick_note_to_document(note_id: str, db: Session = Depends(get_db)):
    note = db.query(QuickNote).filter(QuickNote.id == note_id).first()
    if not note:
        raise HTTPException(status_code=404, detail="Quick note not found")
...
def create_quick_note_group(
    ...
    if note_ids:
        db.query(QuickNote).filter(QuickNote.id.in_(note_ids)).update(
            {"group_id": group_id, "group_name": group_name}, synchronize_session=False
        )
        db.commit()
