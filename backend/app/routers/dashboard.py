...
def get_country_distribution(db: Session = Depends(get_db)):
    result = db.query(
        Country.name,
        Country.id,
        func.count(BookDocument.id).label('book_count')
    ).outerjoin(
        BookDocument, 
        (BookDocument.country_id == Country.id) | 
        (BookDocument.content_region_id == Country.id) |
        (BookDocument.author_region_id == Country.id)
    ).group_by(Country.id, Country.name).having(
        func.count(BookDocument.id) > 0
    ).order_by(func.count(BookDocument.id).desc()).limit(10).all()
...
def get_activity_heatmap(db: Session = Depends(get_db)):
    ...
    events = db.query(DocumentTimelineEvent).filter(
        DocumentTimelineEvent.created_at >= one_year_ago
    ).all()
...
def get_monthly_stats(db: Session = Depends(get_db)):
    ...
    events = db.query(DocumentTimelineEvent).filter(
        DocumentTimelineEvent.created_at >= six_months_ago
    ).all()
