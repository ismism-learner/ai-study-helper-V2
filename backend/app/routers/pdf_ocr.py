...
def resolve_file_path(file_path: str) -> str:
    print(f"[RESOLVE] 解析文件路径: {file_path}")
    file_path = file_path.replace("/", os.sep).replace("\\", os.sep)

    if os.path.isabs(file_path):
        if os.path.exists(file_path):
            print(f"[RESOLVE] 绝对路径存在，返回: {file_path}")
            return file_path

    backend_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
    print(f"[RESOLVE] backend_dir: {backend_dir}")
    ...
    print(f"[RESOLVE] normalized_path: {normalized_path}")
    ...
    for path in unique_paths:
        ...
        print(f"[RESOLVE] 检查路径: {path} -> 存在: {exists}")
        if exists:
            print(f"[RESOLVE] ✅ 找到存在的路径: {path}")
            return path

    final_path = os.path.join(backend_dir, "uploads", "books", normalized_path)
    print(f"[RESOLVE] ⚠️ 未找到存在的路径，返回默认: {final_path}")
    return final_path
...
async def check_ocr_status(file_path: str):
    ...
    return CheckOCRStatusResponse(
        has_text_layer=has_text, total_pages=total_pages, needs_ocr=not has_text
    )
...
async def get_available_languages():
    ...
    for lang in languages:
        result.append(
            {"code": lang, "name": pdf_ocr_service.get_language_display_name(lang)}
        )
...
async def process_pdf_ocr(request: ProcessPDFOCRRequest):
    ...
    if not os.path.exists(resolved_path):
        raise HTTPException(
            status_code=404, detail=f"PDF 文件不存在: {request.file_path}"
        )
...
async def process_pdf_ocr_async(
    ...
    OCR_STATUS_CACHE[cache_key] = {"status": "processing", "progress": 0, "error": None}

    async def process_task():
        try:
            result = await pdf_ocr_service.process_pdf_in_place(
                resolved_path, language=language, deskew=deskew, backup=backup
            )
    ...
    return {"message": "OCR 处理已启动", "file_path": file_path, "status": "processing"}
...
async def load_paddleocr_model(background_tasks: BackgroundTasks):
    ...
    return {
        "message": "PaddleOCR 模型加载已启动，请稍后...",
        "status": paddleocr_service.get_status(),
    }


@router.post("/paddle/process-pdf/{file_path:path}", response_model=PaddleOCRResponse)
async def process_pdf_with_paddle(
    file_path: str, start_page: int = 0, end_page: int = None
):
    resolved_path = resolve_file_path(file_path)
    ...
    result = await paddleocr_service.process_pdf(
        resolved_path, start_page=start_page, end_page=end_page
    )
...
@router.post(
    "/paddle/process-image/{image_path:path}", response_model=PaddleOCRResponse
)
...
@router.post(
    "/paddle/create-searchable/{file_path:path}", response_model=SmartOCRResponse
)
    ...
    async def process_task():
        try:
            ...
            async def progress_callback(
                ...
                IN_PLACE_STATUS_CACHE[cache_key]["message"] = (
                    f"正在处理第 {current_page}/{total_pages} 页..."
                )
    ...
    async def process_task():
        try:
            ...
            async def progress_callback(
                ...
                print(
                    f"[OCR TASK] 进度: {progress}%, 页面 {current_page}/{total_pages}"
                )
                ...
                EXTRACT_TEXT_STATUS_CACHE[cache_key]["message"] = (
                    f"正在处理第 {current_page}/{total_pages} 页..."
                )
            ...
            print(
                f"[OCR TASK] text_content 长度: {len(result.get('text_content', ''))}"
            )
...
async def get_ocr_text_file(file_path: str):
    ...
    base, ext = os.path.splitext(resolved_path)
    text_file_path = f"{base}_ocr_text.txt"
    print(f"[GET OCR] OCR文本文件路径: {text_file_path}")
...
async def check_has_ocr_text(file_path: str):
    ...
    base, ext = os.path.splitext(resolved_path)
    text_file_path = f"{base}_ocr_text.txt"
    print(f"[HAS OCR] OCR文本文件路径: {text_file_path}")

    has_ocr = os.path.exists(text_file_path)
    print(f"[HAS OCR] 文件存在: {has_ocr}")

    if has_ocr:
        file_stat = os.stat(text_file_path)
        print(f"[HAS OCR] 文件大小: {file_stat.st_size}")
        return {
            "has_ocr_text": True,
            "text_file_path": text_file_path,
    ...
    return {"has_ocr_text": False, "text_file_path": None}
...
async def save_ocr_text(file_path: str, text_content: str = Body("", embed=True)):
    ...
    base, ext = os.path.splitext(resolved_path)
    text_file_path = f"{base}_ocr_text.txt"
    print(f"[SAVE OCR] OCR文本文件路径: {text_file_path}")
    print(f"[SAVE OCR] 文本内容长度: {len(text_content)}")
    print(f"[SAVE OCR] 文本内容前100字符: {text_content[:100] if text_content else '(空)'}")
...
async def delete_ocr_text(file_path: str):
    ...
    return {"success": False, "message": "未找到 OCR 文件"}
...
async def get_cache_status():
    ...
    return {
        "extract_text_cache": {
            ...
            "statuses": {
                k: v.get("status") for k, v in EXTRACT_TEXT_STATUS_CACHE.items()
            },
        },
        "in_place_cache": {
